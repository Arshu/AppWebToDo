...
var maxTimeInfo = 5;
function showAjaxTiming(startTime, returnAjax, clear, httpTiming) {
    if (returnAjax.hasOwnProperty('Time') === true) {
        ...
        if (ajaxTimeElm) {
            ...
            if (returnAjax.hasOwnProperty('ServiceInfo') === true) {
                ajaxTime = "(<span title='" + returnAjax.ServiceInfo + "(ajax/http/client)'>" + returnAjax.Time + "ms" + httpTiming +"-"+ timeDiff + "ms</span>)"
            }
        ...
        if (http.readyState == 4) {
            ...
            if (http.status == 200) {
                if (http.responseText.length > 0) {
                    ...
                    //X-Timing	[2.0005ms]
                    var httpTiming = http.getResponseHeader("X-Timing");
                    showAjaxTiming(startTime, response, false, httpTiming);
                }
            }
...
{
    var dateStr = padStr(tempDate.getFullYear()) + '/' + padStr(1 + tempDate.getMonth()) + '/' + padStr(tempDate.getDate()) + ' ' + padStr(tempDate.getHours()) + ':' + padStr(tempDate.getMinutes()) + ':' + padStr(tempDate.getSeconds());
    return dateStr;
}
...
function showMessage(message, msgLabelId, showTime) {
    var msgLabelElm = document.getElementById(msgLabelId);
    if (msgLabelElm) {
